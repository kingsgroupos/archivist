...
Note: To use [.js](#.js) as config file, you must have <a href="https://nodejs.org/en/" target="_blank">node.js</a> installed.
...
Somethings, config data comes from another source other than the file system. In such case, you can organize this kind of data with `archivist.Overwrite` and pass them to `LoadCollection`. `archivist.Overwrite` is always applied at the latest stage.
...
Note: To use .js as config file, you must have <a href="https://nodejs.org/en/" target="_blank">node.js</a> installed.
...
JSON is simple yet good enough. It is widely supported by all kinds of programming languages. It supports hierarchical data structure. It provides no choice between field and property when you add a piece of data into a struct. It is not too hard to compare and merge.

JSON is not perfect. It has too many double quotation marks. It does not support comment. The last item in a {}/[] must have no trailing comma. That is why archivist also supports Javascript file. The data definition syntax of Javascript is very close to JSON.
...
Here is hello-world-like code template of `struct.tpl`:
